...
import { Search, MapPin, Star, Shield, Award, Filter, Building, Loader2, ChevronDown } from "lucide-react";
import { useEffect, useState, useCallback } from "react";
import Link from "next/link";
import { getSuppliers, testSupabaseConnection, type SupplierWithProducts } from "@/lib/api/suppliers";
...
export default function SuppliersPage() {
  ...
  useEffect(() => {
    ...
    initializePage();
  }, [currentPage, connectionTested, searchQuery, loadSuppliers]);
...
function SupplierCard({ supplier }: { supplier: SupplierWithProducts }) {
  return (
    <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 p-4 sm:p-6 border border-slate-200 group">
      <div className="flex items-start gap-3 sm:gap-4 mb-4">
        <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-indigo-100 to-cyan-100 rounded-xl flex items-center justify-center flex-shrink-0">
          <Building className="h-6 w-6 sm:h-8 sm:w-8 text-indigo-600" />
        </div>
        <div className="flex-1 min-w-0">
          <Link href={`/suppliers/${supplier.id}`}>
            <h3 className="text-lg sm:text-xl font-semibold text-slate-900 mb-2 hover:text-indigo-600 cursor-pointer transition-colors group-hover:text-indigo-600 line-clamp-1">
              {stripHtmlTags(supplier.name)}
            </h3>
          </Link>
          ...
          {supplier.address && (
            <div className="flex items-center gap-2 text-slate-500 mb-2">
              ...
              <span className="text-sm line-clamp-1">
                {stripHtmlTags(supplier.address.split(',').slice(-2).join(',').trim())}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm text-slate-600">
              ({supplier.score?.toFixed(1) || 'N/A'})
            </span>
            <span className="text-sm text-slate-500">•</span>
            <span className="text-sm text-slate-500">{supplier.product_count || 0} products</span>
          </div>
        </div>
